package com.spdfnerd.mtr.packet;

import com.spdfnerd.mtr.data.*;
import io.netty.buffer.Unpooled;
import mtr.data.*;
import com.spdfnerd.mtr.gui.ClientData;
import com.spdfnerd.mtr.gui.DashboardScreen;
import com.spdfnerd.mtr.gui.PlatformScreen;
import com.spdfnerd.mtr.gui.ScheduleScreen;
...
public class PacketTrainDataGuiClient implements IPacket {
	...
	public static void receiveTrainsS2C(PacketByteBuf packet) {
		ClientData.trains = receiveData(packet, Train::new);
	}
	...
	public static void sendStationsAndRoutesC2S(Set<Station> stations, Set<Route> routes) {
		...
		sendData(packet, stations);
		sendData(packet, routes);
	...
	public static void receiveAll(PacketByteBuf packet) {
		ClientData.stations = receiveData(packet, Station::new);
		ClientData.platforms = receiveData(packet, Platform::new);
		ClientData.routes = receiveData(packet, Route::new);
