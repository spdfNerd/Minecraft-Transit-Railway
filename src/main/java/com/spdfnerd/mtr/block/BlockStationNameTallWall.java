...
public class BlockStationNameTallWall extends BlockStationNameTallBase {
	...
	public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
		...
		return getVoxelShapeByDirection(2, bounds.getLeft(), 0, 14, bounds.getRight(), 0.5, IBlock.getStatePropertySafe(state, FACING));
	}
	...
	public BlockState getPlacementState(ItemPlacementContext ctx) {
		...
		return isReplaceable(ctx, Direction.UP, 3) ? getDefaultState().with(FACING, facing).with(METAL, true).with(THIRD, EnumThird.LOWER) : null;
	}
