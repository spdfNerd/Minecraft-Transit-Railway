...
public class BlockPSDTop extends HorizontalFacingBlock implements BlockEntityProvider, IBlock, IPropagateBlock {
	...
	public static BlockState getActualState(WorldAccess world, BlockPos pos) {
		...
		return (oldState.getBlock() instanceof BlockPSDTop ? oldState : com.spdfnerd.mtr.Blocks.PSD_TOP.getDefaultState()).with(DOOR_LIGHT, doorLight).with(FACING, facing).with(SIDE_EXTENDED, side).with(AIR_LEFT, airLeft).with(AIR_RIGHT, airRight);
	}
