...
public abstract class BlockPSDAPGDoorBase extends BlockPSDAPGBase {
	...
	public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
		...
		if (IBlock.getStatePropertySafe(state, SIDE) == EnumSide.RIGHT) {
			offsetPos = offsetPos.offset(getSideDirection(state));
		}
		onBreakCreative(world, player, offsetPos);
	...
	public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
		...
		if (open > 0) {
			...
			return getVoxelShapeByDirection(side == EnumSide.LEFT ? 0 : open1, 0, 0, side == EnumSide.RIGHT ? 16 : open2, height, 4, IBlock.getStatePropertySafe(state, FACING));
		} else {
